...
```{r}
#| label: Part 1c
flight_columns <- dbGetQuery(air_con, "PRAGMA table_info('flights');")
print(flight_columns$name)
...
avg_distance <- dbGetQuery(air_con,
                           "SELECT AVG(DISTANCE) AS avg_dis
                           FROM flights
                           WHERE YEAR = 2015")
print(paste("1:", avg_distance$avg_dis, "miles"))

min_distance <- dbGetQuery(air_con,
                           "SELECT MIN(DISTANCE) AS min_dis
                           FROM flights
                           WHERE YEAR = 2015")
print(paste("2:", min_distance$min_dis, "miles"))

max_distance <- dbGetQuery(air_con,
                           "SELECT MAX(DISTANCE) AS max_dis
                           FROM flights
                           WHERE YEAR = 2015")
print(paste("3:", max_distance$max_dis, "miles"))

min_dist_airports <- dbGetQuery(air_con,
                                    "SELECT ORIGIN_AIRPORT, DESTINATION_AIRPORT
                                    FROM flights
                                    WHERE DISTANCE = (SELECT MIN(DISTANCE)
                                    FROM flights WHERE YEAR = 2015)
                                    AND YEAR = 2015")
print(paste("4:", min_dist_airports$ORIGIN_AIRPORT, "to",
            min_dist_airports$DESTINATION_AIRPORT))

max_dist_airports <- dbGetQuery(air_con,
                                    "SELECT DISTINCT
                                     CASE
                                    WHEN ORIGIN_AIRPORT < DESTINATION_AIRPORT
                                    THEN ORIGIN_AIRPORT ELSE DESTINATION_AIRPORT
                                    END AS port1,
                                    CASE
                                    WHEN ORIGIN_AIRPORT > DESTINATION_AIRPORT
                                    THEN ORIGIN_AIRPORT ELSE DESTINATION_AIRPORT
                                    ...
                                    FROM flights WHERE YEAR = 2015)
                                    AND YEAR = 2015")
print(paste("5:", max_dist_airports$port1, "to",
            max_dist_airports$port2))

max_dist_airports_2 <- dbGetQuery(air_con,
                                    "SELECT DISTINCT 
                                    CASE
                                    WHEN ORIGIN_AIRPORT < DESTINATION_AIRPORT
                                    THEN ORIGIN_AIRPORT ELSE DESTINATION_AIRPORT
...
print(paste("6:", max_dist_airports_2$port1, "to",
            max_dist_airports_2$port2))

min_airport_name <- dbGetQuery(air_con,
                      "SELECT a.AIRPORT AS dep_name,
                      b.AIRPORT AS arriv_name
                      FROM flights c
                      INNER JOIN airports a ON c.ORIGIN_AIRPORT = a.IATA_CODE
                      INNER JOIN airports b ON c.DESTINATION_AIRPORT = b.IATA_CODE
                      WHERE c.DISTANCE = (SELECT MIN(DISTANCE)
                      FROM flights WHERE YEAR = 2015)
                      AND c.YEAR = 2015")
print(paste("7:", min_airport_name$dep_name, "to", min_airport_name$arriv_name))

max_airport_name <- dbGetQuery(air_con,
                      "SELECT DISTINCT
                      CASE
                      WHEN c.ORIGIN_AIRPORT < c.DESTINATION_AIRPORT
                      THEN a.AIRPORT ELSE b.AIRPORT
                        END AS dep_name,
                      CASE
                      WHEN c.ORIGIN_AIRPORT > c.DESTINATION_AIRPORT
                      THEN a.AIRPORT ELSE b.AIRPORT
                      ...
                      INNER JOIN airports a ON c.ORIGIN_AIRPORT = a.IATA_CODE
                      INNER JOIN airports b ON c.DESTINATION_AIRPORT = b.IATA_CODE
                      WHERE c.DISTANCE = (SELECT MAX(DISTANCE)
                      FROM flights WHERE YEAR = 2015)
                      AND c.YEAR = 2015")
print(paste("8:", max_airport_name$dep_name, "to", max_airport_name$arriv_name))
...
flights_tbl <- tbl(air_con, "flights")

dbDisconnect(air_con)
```