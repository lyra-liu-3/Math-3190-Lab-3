...
car_price_prediction <- car_price_prediction |> rename(car_year = year)

car_price_pred_log <- car_price_prediction |>
  mutate(logselling_price = log(selling_price)) |>
  mutate(logyear = log(car_year)) |>
  mutate(logkm_driven = log(km_driven)) |>
  filter(fuel != 'Electric')

carp_model <- lm(logselling_price ~ . -name - car_year - km_driven - selling_price,
                 data = car_price_pred_log)
summary(carp_model)
```
...
price_lasso <- glmnet(logselling_price ~ . -name, data = car_price_pred_log,
                      alpha = 1)
coef(price_lasso, s = 0)
coef(price_lasso, s = 0.01)
coef(price_lasso, s = 0.1)
coef(price_lasso, s = 1)
```
...
price_ridge <- glmnet(logselling_price ~ . -name, data = car_price_pred_log,
                      alpha = 0, )
coef(price_ridge, s = 0)
coef(price_ridge, s = 0.01)
coef(price_ridge, s = 0.1)
coef(price_ridge, s = 1)
```
For lambda = 0 and 0.01 the coefficients are very similar, and 0.1 is also fairly similar. At 1 there is a big change in most of the variables. 
...
price_elastic <- glmnet(logselling_price ~ . -name, data = car_price_pred_log,
                      alpha = 1/3)
coef(price_ridge, s = 0)
coef(price_ridge, s = 0.01)
coef(price_ridge, s = 0.1)
...
# LASSO
cvprice_lasso <- cv.glmnet(logselling_price ~ . -name, data = car_price_pred_log,
                      alpha = 1)
fit_price_lasso <- glmnet(logselling_price ~ . -name, data = car_price_pred_log,
                      alpha = 1, lambda = cvprice_lasso$lambda.1se)

# Ridge
cvprice_ridge <- cv.glmnet(logselling_price ~ . -name, data = car_price_pred_log,
                      alpha = 0)
fit_price_ridge <- glmnet(logselling_price ~ . -name, data = car_price_pred_log,
                      alpha = 0, lambda = cvprice_ridge$lambda.1se)

#Elastic
cvprice_elastic <- cv.glmnet(logselling_price ~ . -name, data = car_price_pred_log,
                      alpha = 1/3)
fit_price_elastic <- glmnet(logselling_price ~ . -name, data = car_price_pred_log,
                      alpha = 1/3, lambda = cvprice_elastic$lambda.1se)
```
...
nboot <- 1000
coefs <- matrix(rep(0, nboot*16), nrow = nboot)
for(i in 1:nboot){
  index <- sample(1:nrow(car_price_pred_log), nrow(car_price_pred_log),
                  replace = T)
  boot_data <- car_price_pred_log[index, ]
  mod <- cv.glmnet(logselling_price ~ . -name, data = boot_data,
                      alpha = 0)
  coefs[i,] <- coef(mod, s = "lambda.1se")[1:16]
}
...
house_linreg <- lm(house_value ~ ., data = housing_train)
```
...
#| label: Part 2 b
set.seed(2002)
cvhouse_ridge <- cv.glmnet(house_value ~ ., data = housing_train,
                      alpha = 0)
fit_house_ridge <- glmnet(logselling_price ~ . -name, data = car_price_pred_log,
                      alpha = 0, lambda = cvhouse_ridge$lambda.min)
```
...
Ship_data <- ShipAccidents |> filter(service != '0')

ship_poisson <- glm(incidents ~ ., data = Ship_data, family = "poisson")
ship_poisson
```
We are predicting incidents, which is a discrete, quantitative response variable where large counts are rare, which is what Poisson regression is best for as defined in the notes.

### Part b (5 points)
Using that model, interpret the slope of "service" and the slope of "construction1970-74" in original (not log) units.
```{r}
exp(coef(ship_poisson))
```
As service increases by one unit (one month of service), all other variables held constant, we would predict the incidents to increase by 1. Ships built in 1970-74 have 4.24% more incidents, all other variables held constant.
...
graph_frame <- data.frame(
    pearson_resids = residuals(ship_poisson, type = "pearson"),
      linear_pred = predict(ship_poisson, type = "link"))

ggplot(graph_frame, aes(x = linear_pred, y = pearson_resids)) + geom_point() +
     labs(title= "Pearson residuals vs linear predictors",
                        x = "Linear Predictors",
                        y = "Pearson Residuals")

ggplot(graph_frame, aes(sample = pearson_resids)) + geom_qq_line() + geom_qq()
    
```
The first plot seems to be mostly randomly scattered which is good but the QQ plot has tails which indicates it is not a good fit.
